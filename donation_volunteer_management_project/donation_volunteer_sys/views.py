    ...
    def post(self,request):
        ...
        if form1.is_valid() and form2.is_valid():
            ...
            try:
                ...
                messages.success(request,"Congratualations!! Donor Profile Created Successfully")
...
def index_admin(request):
    ...
    totaldonars=Donation.objects.all().count()

    totalvolunteers=Donation.objects.all().count()
    ...
    totaldonationsareas=Donation.objects.filter().count()
            ...
            try:
                if newpass == confirmpass:
                    ...
                    if user.check_password(old):
                        ...
                        messages.warning(request,"Old Password dose not match")
                else:
...
class accepted_donationdetail(View):
    ...
    def post(self, request, pid):
        ...
        try:
            ...
            donation.volunteerremark= "Volunteer Allocated by Admin"
            donation.updationdate = date.today()
            donation.save()
            messages.success(request, "Donation Area & Volunteer Allocated Successfully")
        except:
            messages.warning(request, "Failed to allocate Donation Area & Volunteer")
...
class view_donationdetail(View):    
    ...
    def post(self, request, pid):   
        ...
        try:
            ...
            messages.success(request, "Failed to update status & remark")       
...
class donate_now(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():  
            ...
            description= request.POST['description'] 
            try:
                Donation.objects.create(donor=donor, donationname=donationname, 
                donationpic=donationpic, collectionloc=collectionloc, description=description,
                status="pending", donationdate=date.today())

                messages.success(request,"Congratualations Donation save Successfully")
...
def donation_history(request):
    if not request.user.is_authenticated:
        return redirect.user('/login-donor')
    user=request.user
    donor=Donor.objects.get(user=user)
...
class profile_donor(View):
    ...
    def post(self,request):
        ...
        try:
            ...
            messages.success(request,"Congratualations!! Donor Profile Updated Successfully")
...
class changepwd_donor(View):
    ...
    def post(self,request):
        ...
        confirmpass=request.POST.get('new_password2')
        try:
            if newpass == confirmpass:
                user=User.objects.get(id=request.user.id)
                if user.check_password(old):
...
class profile_volunteer(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            messages.success(request, "Congratulations! Volunteer Profile Updated Successfully")
